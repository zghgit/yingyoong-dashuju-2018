<template>
  <section class="constWidth" id="fifteenCommon">
    ...
    <Modal id="lookInfoPop" title="股票质押式回购会员报送初始交易明细报表  统计口径说明" v-model="changeInfo"
           class-name="vertical-center-modal"
           cancel-text>
      <h3>一．数据说明</h3>
      <p>
        1.本应用可查询时间起始范围为：2015.12.10-now
        <br>2.本应用可.查询T日数据的时点为：22：00之后
      ...
      <p>
        1.盯市日期: T-1日日期
        <br>2.业务类型:初始交易或补充质押
        <br>3.初始交易日期: 初始交易申报日期；对于补充质押，对应初始交易的申报日期。
        <br>4.初始交易成交编号: 对于补充质押，对应初始交易成交编号
        <br>5.补充质押日期: 如属于补充质押则为非空，如属于初始交易则为空
        <br>6.补充质押成交编号: 如属于补充质押则为非空，如属于初始交易则为空
        <br>7.证券代码: 质押标的证券代码
        <br>8.证券简称: 质押标的证券简称
        <br>9.是否属于限售股份: 0：否；1：是
        <br>10.标的证券数量: 对于初始交易，为初始交易质押标的证券数量；对于补充质押，为补充质押标的证券数量
        <br>11.初始交易金额: 初始交易成交的金额
        <br>12.初始购回期限: 初始交易时约定的购回期限
        <br>13.初始购回日期: 初始交易时约定的购回日期
        <br>14.初始购回金额: 初始交易时约定的购回金额
        <br>15.融资利率:为初始交易时约定的融资利率
        <br>16.融出方属性: 00：自有资金；01：集合资产管理计划；02：定向资产管理计划；03：其他
        <br>17.融入方属性: 0：个人投资者；1：机构投资者
        <br>18.融入方应付金额: 根据场外分期付息等情况调整；对于当日了结的合约，展示该笔合约实际的购回金额
        <br>19.实际融资利率: 对于未了结的合约，展示该笔合约初始交易时约定的融资利率；对于当日了结的合约，展示该笔合约实际的融资利率
        <br>20.当前质押数量: 标的证券当前的质押在押数量，包括质押红股数量。从初始交易日（或补充质押日）起至T-1日（含）期间，该笔初始交易（或补充质押交易）标的证券数量与累计权益分派所获得的红股数量之和减去累计部分解除质押的标的证券数量。对于当日了结的合约，填报该笔合约实际的购回数量
        <br>21.当前质押红利金额: 从初始交易日（或补充质押日）起至T-1日（含）期间，所有权益分派所获得的红利金额之和减去累计部分解除质押的红利金额
        <br>22.盯市履约保障比例: 指单笔交易或合并管理的多笔交易的质押标的证券市值与质押红利金额之和与融入方应付金额的比值；质押标的证券涉及沪深两市的，合并计算履约保障比例
        <br>23.履约保障级别: 0：正常；1：警戒；2：触发
        <br>24.合约状态: 0：未到期；1：已到期未了结；2：当日已了结；9：违约处置中
        <br>25.了结类型: 00：未了结；01：提前购回；02：到期购回；03：延期购回；04：终止购回；09：违约处置了结
        <br>26.资金用途类型: 00：消费；01：企业经营；02：新股申购；03：定向增发；04：二级市场股票交易；05：其他
        <br>27.资金用途描述: 若资金用途类型字段为其他，则本字段为对融入方获得资金的后续用途进行简要描述
        <br>28.质押标的价格: 为对应标的证券当日收盘价，当日无收盘价的，取最近一交易日收盘价
        <br>29.质押标的市值: 通过股票质押回购盯市报告数据文件中的当前质押数量*质押标的价格（质押标的市值=当前质押数量*质押标的价格）
      </p>
      <h3>三．其他说明</h3>
      <p>
        1）在当日发送时间截止之前，如发现数据漏发或错发，可重新打包上传，包内文件必须为全量数据；
        <br>2）T日上传时，除T-1日股票质押回购盯市报告数据外，如还包含T-1日之前的盯市报告数据，则需要根据具体日期，将相应的数据放入相应的文件中，文件命名规则同上。例如，如还包括T-2日的盯市数据，则将T-2日的数据保存在zyhgXXXXX001[T-1].txt中,并生成对应标志文件。所有数据文件就绪后，打包至rar压缩文件包进行上传。
      </p>
    </Modal>
    ...
    <!--<h2 class="infoTitle">股票质押式回购会员报送初始交易明细报表</h2>-->
      ...
      <!--<Row :gutter="gutterValue">-->
        ...
        <!--<Card class="iviewCard">-->
          ...
          <!--<Modal id="lookInfoPop" title="股票质押式回购会员报送初始交易明细报表  统计口径说明" v-model="changeInfo"-->
                 ...
                 <!--cancel-text>-->
            <!--<h3>数据说明</h3>-->
            <!--<p>包含会员股票质押式回购交易信息明细数据</p>-->
            <!--<h3>指标说明</h3>-->
            <!--<p>-->
              <!--1.盯市日期: T-1日日期<br>-->
              <!--2.业务类型:初始交易或补充质押<br>-->
              <!--3.初始交易日期: 初始交易申报日期；对于补充质押，对应初始交易的申报日期。<br>-->
              <!--4.初始交易成交编号: 对于补充质押，对应初始交易成交编号<br>-->
              <!--5.补充质押日期: 如属于补充质押则为非空，如属于初始交易则为空<br>-->
              <!--6.补充质押成交编号: 如属于补充质押则为非空，如属于初始交易则为空<br>-->
              <!--7.证券代码: 质押标的证券代码<br>-->
              <!--8.证券简称: 质押标的证券简称<br>-->
              <!--9.是否属于限售股份: 0：否；1：是<br>-->
              <!--10.标的证券数量: 对于初始交易，为初始交易质押标的证券数量；对于补充质押，为补充质押标的证券数量<br>-->
              <!--11.初始交易金额: 初始交易成交的金额<br>-->
              <!--12.初始购回期限: 初始交易时约定的购回期限<br>-->
              <!--13.初始购回日期: 初始交易时约定的购回日期<br>-->
              <!--14.初始购回金额: 初始交易时约定的购回金额<br>-->
              <!--15.融资利率:为初始交易时约定的融资利率<br>-->
              <!--16.融出方属性: 00：自有资金；01：集合资产管理计划；02：定向资产管理计划；03：其他<br>-->
              <!--17.融入方属性: 0：个人投资者；1：机构投资者<br>-->
              <!--18.融入方应付金额: 根据场外分期付息等情况调整；对于当日了结的合约，展示该笔合约实际的购回金额<br>-->
              <!--19.实际融资利率: 对于未了结的合约，展示该笔合约初始交易时约定的融资利率；对于当日了结的合约，展示该笔合约实际的融资利率<br>-->
              <!--20.当前质押数量:-->
              <!--标的证券当前的质押在押数量，包括质押红股数量。从初始交易日（或补充质押日）起至T-1日（含）期间，该笔初始交易（或补充质押交易）标的证券数量与累计权益分派所获得的红股数量之和减去累计部分解除质押的标的证券数量。对于当日了结的合约，填报该笔合约实际的购回数量<br>-->
              <!--21.当前质押红利金额: 从初始交易日（或补充质押日）起至T-1日（含）期间，所有权益分派所获得的红利金额之和减去累计部分解除质押的红利金额<br>-->
              <!--22.盯市履约保障比例: 指单笔交易或合并管理的多笔交易的质押标的证券市值与质押红利金额之和与融入方应付金额的比值；质押标的证券涉及沪深两市的，合并计算履约保障比例<br>-->
              <!--23.履约保障级别: 0：正常；1：警戒；2：触发<br>-->
              <!--24.合约状态: 0：未到期；1：已到期未了结；2：当日已了结；9：违约处置中<br>-->
              <!--25.了结类型: 00：未了结；01：提前购回；02：到期购回；03：延期购回；04：终止购回；09：违约处置了结<br>-->
              <!--26.资金用途类型: 00：消费；01：企业经营；02：新股申购；03：定向增发；04：二级市场股票交易；05：其他<br>-->
              <!--27.资金用途描述: 若资金用途类型字段为其他，则本字段为对融入方获得资金的后续用途进行简要描述<br>-->
              <!--28.质押标的价格: 为对应标的证券当日收盘价，当日无收盘价的，取最近一交易日收盘价<br>-->
              <!--29.质押标的市值: 通过股票质押回购盯市报告数据文件中的当前质押数量*质押标的价格（质押标的市值=当前质押数量*质押标的价格）<br>-->
            <!--</p>-->
            <!--<h3>其他说明</h3>-->
            <!--<p>-->
              <!--1）在当日发送时间截止之前，如发现数据漏发或错发，可重新打包上传，包内文件必须为全量数据；<br>-->
              <!--2）T日上传时，除T-1日股票质押回购盯市报告数据外，如还包含T-1日之前的盯市报告数据，则需要根据具体日期，将相应的数据放入相应的文件中，文件命名规则同上。例如，如还包括T-2日的盯市数据，则将T-2日的数据保存在zyhgXXXXX001[T-1].txt中,并生成对应标志文件。所有数据文件就绪后，打包至rar压缩文件包进行上传。<br>-->
            <!--</p>-->
          <!--</Modal>-->
          <!--&lt;!&ndash;头部logo&ndash;&gt;-->
          <!--<p id="infoTableList" class="redtext listlength"></p>-->
            ...
            <!--<Radio-group v-model="formValidate.cxtype" class="zqradio">-->
              <!--<row>-->
                <!--<div id="zqcode">-->
                  <!--<section class="warp">-->
                    <!--<div class="searchIpt clearFix">-->
                      <!--<input @blur="showListHandle" :value="titles"-->
                             <!--v-model="titles" placeholder="请输入/选择会员名称"-->
                             <!--class="select-input"/>-->
                      <!--<div class="select-down dropDown" v-show="listShow">-->
                        <!--<ul class="list">-->
                          <!--<li class="li-item" v-for="item in zqCodeList"-->
                              <!--@click="getTitleHandle(item.value)">{{-->
                            <!--item.label }}-->
                          <!--</li>-->
                        <!--</ul>-->
                      <!--</div>-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/GDH_00015.js';
  ...
  export default {
    ...
    methods: {
      ...
      formatData (row, column, cellValue) {
        ...
        if (prop == 'trade_vol' || prop == 'init_trade_amt' || prop == 'init_trade_amt' || prop == 'repur_term' || prop == 'repur_amt' || prop == 'pay_amt' || prop == 'impawn_vol' || prop == 'dividend_amt' || prop == 'pre_curr_price' || prop == 'vol_price') {
