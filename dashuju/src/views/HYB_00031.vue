...
<template>
  <section class="constWidth" id="hyb00031">
    <div id="fifteenCommon">
      ...
      <Modal id="lookInfoPop" title="约定购回及股票质押履约保障比例(二)  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
             cancel-text>
        1. 只有董监高账号或者交易前持股大于5%的且持有约定购回或者股票质押未到期的账号才予于展示。
        <br>2. 根据中登系统提供的未到期数据，统计约定购回和股票质押的待购回初始交易金额，统计待购回初始交易金额时，仅统计在非担保交收中“交收成功”的数据。
        <br>3. 根据当日行情取出标的证券简称。
        <br>4. 持股比例（交易前）（%） = [该账户总持有（流通+非流通）+约定购回待购回数量]*100 /该股票总股本（流通+非流通）
        <br>5. 股票质押调整后标的证券数量=(T-1日调整后标的证券数量)*(1+T日为除权除息日的送股比例、转增比例)-T日部分解质数量。
        <br>6. 履约保证比例=标的证券市值/待购回初始交易金额
        <br>7. 调整后待购回初始交易金额=待购回初始交易金额-待购回初始交易金额变动
        <br>8. 调整后履约保证比例(%) = 调整后担保物的价值/调整后待购回初始交易金额
        <br>9. 调整后担保物价值 = 担保物价值 + 其他担保物价值。
        <br>10. 通常情况下，对于约定购回业务的数据，在T+1日20:00才可获得T日统计数据。对于股票质押业务的数据，在T日20:00其可获得T日的统计数据。
        <br>11. PK字段为融入方账号、标的证券代码、交易日期。
      </Modal>
      <!--头部logo-->
      <div class="tableBox">
...
<script>
  ...
  import {tableColumns1} from '../assets/js/HYB_00031.js';
  ...
  export default {
    ...
    methods: {
      ...
      fn_formatter:function (row,col,cellValue) {
        ...
        // 需要格式化千分位的字段
        if(prop =='ydgh_up_trade_amt' || prop == 'ydgh_up_other_amt'|| prop == 'gpzy_up_trade_amt'|| prop == 'gpzy_up_other_amt'|| prop == 'gpzy_up_other_amt'|| prop == 'hj_up_other_amt'|| prop == 'hj_up_trade_amt'){
      ...
      exportData() {
        ...
        var dp = '/dw/download/hyb00031Export?'
