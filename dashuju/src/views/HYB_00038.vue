<template>
  <section class="constWidth" id="fifteenCommon">
    ...
    <Modal id="lookInfoPop" title="约定购回及股票质押期间质押率  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal" cancel-text>
      <b>一．	整体说明</b>
      <br>1. 统计期间所有约定购回和股票质押交易明细数据每笔的质押率，仅统计交收成功的数据。
      <br>2. 对于股票质押业务，根据综业平台成交数据明细的“扩展标识”前9位有效数字，第1-2位表示证券类别。当类别为“PT”时表示为无限售流通股，当类别为 “XL”时表示为限售流通股。
      <br>3. 每笔质押率=初始交易金额 / 质押标的券市值。平均质押率是每笔质押率的算术平均值。
      <br>4. 每次查询只展示一条数据。
    </Modal>
    <div class="tableBox">
    ...
    <!--左右结构代码-->
    <!--<h2 class="infoTitle">约定购回及股票质押期间质押率</h2>-->
    <!--<div class="mainBox changeBox">-->
      <!--<Row :gutter="gutterValue">-->
        ...
        <!--<Card class="iviewCard">-->
          ...
          <!--<Modal id="lookInfoPop" title="约定购回及股票质押期间质押率  统计口径说明" v-model="changeInfo"-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/HYB_00038.js';
  ...
  export default {
    ...
    methods: {
      ...
      publicPagination(pages, rows, statics) {document.getElementById("exportCurrent").setAttribute("disabled",true);
        ...
        this._fetch(fetch(url, {
          ...
          if (rtnStatic == false) {
            ...
            if (dataResponse.length == 0 || empty) {
      ...
      exportData() {
        ...
        void(window.open('/dw/hyb/hyb_00038_export?' + downfileParams + commonGetReqParam));
      },
