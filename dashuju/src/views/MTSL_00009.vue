<template>
  <section class="constWidth" id="mtsl00009">
    <!--头部logo-->
    ...
    <Modal id="lookInfoPop" title="融资融券违约情况  统计口径说明" v-model="changeInfo"
      ...
      4)	证券查询范围：融资融券违约情况<br>
      5)	交易平台：竞价系统<br>
        <b>二．	单指标算法</b><br>
      序号	展示字段	展示字段算法或公式<br>
      1.	未了结违约账户数	统计期间发生的未了结违约的账户数<br>
      2.	未了结违约笔数	统计期间发生的未了结违约笔数<br>
      3.	未了结违约金额	统计期间发生的未了结违约金额<br>
      4.	占比	根据需要的维度，计算占比<br>
          ...
          <el-table-column v-if="item.hasAnchor" :key="index"
            ...
            <template scope="scope">
             {{scope.row.trade_date}}
              <a style="margin-left: 10px;" @click="anchorClick(scope)">会员明细</a>
            </template>
          </el-table-column>
...
<script>
  ...
  import {tableColumns1} from '../assets/js/MTSL_00009.js';
  ...
  export default {
    ...
    methods: {
      ...
      anchorClick:function (scope) {
        jump2NewPage('MTSL_00009_detail',{
          data:{rptDte:scope.row.trade_date},
          title:'会员明细'
        },true)
        console.log(scope)
      },
      ...
      exportData() {
        ...
        var downfileParams = "userId=" + srcUseridA + "&userName=" + srcUsernameA + "&signature=" + signatureA + "&Authorization=" + AuthorizationA + "&startDate=" + this.recordQueryDate.startdate.format('yyyy-MM-dd')+'&endDate='+this.formValidate.enddate.format('yyyy-MM-dd');
        $("#exportCurrent").attr("disabled");
        setTimeout(function () {
          $('#exportCurrent').removeAttr('disabled');
        }, 5000);
        var dp = '/dw/download/mtsl00009export?'
...
<style>
  #mtsl00009 .ivu-collapse-content{
