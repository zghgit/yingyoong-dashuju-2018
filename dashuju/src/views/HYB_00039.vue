<template>
  <section class="constWidth" id="fifteenCommon">
          ...
          <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
            <div class="flexBox">
              <Form-item prop="startdate" id="startDateForm" label="交易日期">
                <Date-picker id="startDate" type="date" @on-open-change="datePickerClose" placeholder="选择交易日期" v-model="formValidate.startdate" class="datePickerWidth"></Date-picker>
              </Form-item>
    ...
    <Modal id="lookInfoPop" title="约定购回及股票质押单证券历史比较统计  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
           cancel-text>
       <b> 一．	整体说明</b>
       <br>1．有约定购回未到期或股票质押未到期的标的证券才纳入统计。
       <br>2．根据中登系统提供的未到期数据，统计约定购回和股票质押的待购回初始交易金额，统计待购回初始交易金额时，仅统计在非担保交收中“交收成功”的数据。
       <br>3．证券名称显示为“证券名称简称”。
       <br>4．过去30交易日成交额 = 该证券在竞价系统中30交易日的二级市场交易金额合计。过去30交易日是指[T,T-29]，包含本交易日，从行情中提取成交金额。
       <br>5．对每只证券，期末收盘价较过去20个交易日最大跌幅 = 1-T日收盘价 / MAX( 过去20交易日的盘中最高价 )。过去20交易日是指[T,T-19]，包含本交易日。统计跌幅时，暂不考虑除权除息的情况。
       <br>6．与过去30交易日成交额比值（%）= 该证券该项业务（约定购回或股票质押或二者合计）待购回初始交易金额 / 该证券过去30交易日成交额。
       <br>7．两项合计的调整后待购回初始交易金额 = 调整后约定购回待购回初始交易金额+调整后股票质押待购回初始交易金额
       <br>8．两项合计的标的证券市值= 约定购回标的证券市值+股票质押标的证券市值
       <br>9．两项合计的与过去30交易日成交额比值（%） = 项合计的标的证券市值 /过去30交易日成交额
       <br>10．页面表格中记录排序方式为：证券代码（正序从小到大）
       <br>11．通常情况下，对于约定购回业务的数据，在T+1日20:00才可获得T日统计数据。对于股票质押业务的数据，在T日20:00其可获得T日的统计数据。
       <br>12．PK字段为证券代码和交易日期。
    </Modal>
    <div class="tableBox">
    ...
    <!--左右结构代码-->
    <!--<h2 class="infoTitle">约定购回及股票质押单证券历史比较统计</h2>-->
          ...
          <!--<Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">-->
            <!--<Row class="dateBox">-->
              ...
              <!--<Form-item prop="startdate" id="startDateForm">-->
                <!--<Date-picker id="startDate" type="date" @on-open-change="datePickerClose" placeholder="选择交易日期"-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/HYB_00039.js';
  ...
  export default {
    ...
    methods: {
      ...
      fn_formatter:function (row,col,cellValue) {
        ...
        // 需要格式化千分位的字段
        if(prop =='sum_trade_amt' || prop == 'close_price_ratio' || prop =='up_trade_amt_1' || prop == 'hold_amt_1'|| prop == 'up_trade_amt_1_ratio' || prop =='up_trade_amt_2' || prop == 'hold_amt_2'|| prop == 'up_trade_amt_2_ratio' || prop =='up_trade_amt_s' || prop == 'hold_amt_s'|| prop == 'up_trade_amt_s_ratio' ){
      ...
      exportData() {
        ...
        var dp = '/dw/hyb/hyb_00039_export?'
