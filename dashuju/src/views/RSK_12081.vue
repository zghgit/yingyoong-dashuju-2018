<template>
  <section id="rsk12081">
    ...
    <Modal id="lookInfoPop" title="风险周报_证券全天停牌  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
           cancel-text>
      <b>一．	整体说明</b>
     <br> 1) 本应用可查询时间起始范围为：2012.11.30-now
     <br> 2) 本应用可查询T日数据的时点为：T日18:00后
     <br> 3) 本应用限制最大查询时间区间为：无限制
      <br><b> 二． 单指标算法：</b>
     <br>序号 展示字段 展示字段算法或公式
     <br>1 停牌时长（天） 按证券品种加总期间所有有停牌的证券的停牌天数之和（只统计期间有全天停牌的证券，并剔除不计入停牌时长的天数。）
     <br>2 停牌证券数量 期间有停牌的证券的数量（只统计期间有全天停牌的证券，并剔除不计入停牌时长的情况），对于其间多次停牌的，其代码只统计一次
     <br>3 两融标的证券停牌时长 有融券卖出/融资买入资格的标的证券期间的停牌天数之和（只统计期间有全天停牌的证券）
     <br>4 两融标的证券停牌数量 有融券卖出/融资买入资格的标的证券期间有停牌的数量，对于其间多次停牌的，其代码只统计一次
     <br>5 上一年度日平均 对上年度全天停牌的停牌时长(天)按交易日计算其平均值
     <br>6 上一年度日标准方差 [( ∑(上年每交易日停牌时长-上年度日平均)^2 ) / 上年交易日数
     <br>7 上一年度周平均 对上年度全天停牌的停牌时长(天)按周计算其平均值
     <br>8 上一年度周标准方差 [ ( ∑(上年每周停牌时长-上年度周平均)^2 ) / 上年周数
     <br>9 上一年度月平均 对上年度全天停牌的停牌时长(天)按月计算其平均值
     <br>10 上一年度月标准方差 [ ( ∑(上年每月停牌时长-上年度月平均)^2 ) / 上年月数
    </Modal>
    <div class="tableBox">
    <p id="infoTableList" class="redtext listlength"></p>
      <el-col :span="24">
        <el-table
            ...
            border
            style="width: 100%">
          <el-table-column v-for="(item,index) in tableColumns1"
           :key="index"
           :prop="item.field"
           :label="item.title"
           :align="item.align"
...
<script>
  ...
  import {tableColumns1} from '../assets/js/RSK_12081.js';
  ...
  export default {
    ...
    methods: {
      ...
      fn_formatter:function (row,col,cellValue) {
        ...
        // 需要格式化千分位的字段
        // if(prop !=='bond_type' || prop !== 'suspension_time' ){   // 不被格式化千分位的字段

        if( prop == 'preyear_week_std' || prop == 'suspension_num' || prop == 'bond_suspension_time' || prop == 'bond_suspension_num' || prop == 'preyear_day_avg' || prop == 'preyear_day_std' || prop == 'preyear_week_avg'|| prop == 'preyear_month_avg'|| prop == 'preyear_month_std'){
      ...
      exportData(type) {
        ...
        document.getElementById("exportCurrent").setAttribute("disabled",true);
          ...
          void(window.open('/dw/download/rskexcel?type=bdsp_rsk12081&row=1&' + downfileParams +commonGetReqParam));
        }
      },
