<template>
  <div class="container">
    ...
    <div class="exportBtn">
      <Button id="exportCurrent" type="primary" size="large" disabled @click="exportData()">
...
<script>
  ...
  export default {
    ...
    mounted() {
      var dataObj = JSON.parse(sessionStorage.getItem('MTSL_00007_date'))
      console.log(dataObj)
      var o = JSON.parse(dataObj.data)
      console.log(o)
      console.log(dataObj.title.indexOf('日'))
      if(dataObj.title.indexOf('日') > -0.5){
        $('.el-checkbox-group label:nth-child(2)').css('display','none')
      }
      if(o.inputCollRatio){
      ...
      // this.tableColumns3 = tableColumns3

      var temp = []
      tableColumns3.forEach((item,index)=>{
        if(!item.flag){
          temp.push(item)
        }
      })
      this.tableColumns3 =  temp
    },
    methods: {
      // 列：格式化字符
...
// 勾选时
      handleChangeTypesChange: function (value) {
        ...
        this.types.forEach((type, index) => {
          value.forEach((t, ind) => {
            if (t == type) {
              ...
              tableColumns3.forEach((o, inde) => {
                if (o.flag == (index + 1)) {
                  this.tableColumns3.push(o)
                }
              })
            }
          })
        ...
        //  排序
        this.tableColumns3.sort((o1,o2)=>{
          return o1.aIndex < o2.aIndex ? -1 : 1
        })
      },
      exportData() {
        var downfileParams = "userId=" + srcUseridA + "&userName=" + srcUsernameA + "&signature=" + signatureA + "&Authorization=" + AuthorizationA + "&inputCollRatio=" +this.dataFromParent.inputCollRatio+'&startDate='+this.dataFromParent.startDate+'&endDate='+this.dataFromParent.endDate+'&showWhich='+this.currentCheckedArr+'&inputData='+this.dataFromParent.inputData
