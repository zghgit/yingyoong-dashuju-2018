<template>
  <section class="constWidth">
    <!--头部logo-->
    <header></header>
    <h2 class="infoTitle">融资融券标的清单</h2>
    <div class="mainBox changeBox" id="fts00012">
      <Row :gutter="gutterValue">
        ...
        <Card class="iviewCard">
          ...
          <Modal id="lookInfoPop" title="融资融券标的清单  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
                 cancel-text>
            <h3>数据说明</h3>
            <p>数据时间范围：界面所选择的日期范围</p>
            <p>证券选择范围：融资融券标的证券</p>
            <p>交易平台：竞价平台</p>
            <h3>指标说明</h3>
            <p>
          ...
          <!--<fastTable :width="1200" :bwidth="1200" :columns="tableColumns1" :data="tableData1"></fastTable>-->
          <el-table
          :data="tableData1"
...
<script>
  ...
  import {tableColumns1} from '../assets/js/FTS_00012.js';
  ...
  export default {
    data() {
      return {
        ...
        changeInfo: false,
        //初始化表格信息
        tableColumns1: tableColumns1[0],
        tableColumns2: tableColumns1[1],
        tableData1: [],
        tableData2: [],
        //分页信息
        returnTotal: 0,
    ...
    methods: {
      ...
      hideRightBox: function () {
        ...
        this.autoFit(this.tableColumns1)
        this.autoFit(this.tableColumns2)
      },
      // 调整两个表格内容自适应
      autoFit:function(table){
        var self = this
        setTimeout(function () {
          var w =  $('.el-table__header-wrapper').width()
      ...
      handleSubmit(name,isPageClick) {
        this.$refs[name].validate((valid) => {
          ...
          if (valid) {
            ...
            this.publicPagination(this.returnN, this.pageSize, true,'001');
            this.publicPagination(this.returnN, this.pageSize, true,'002');
          } else {
       ...
       * 分页、查询功能
       */
      publicPagination(pages, rows, statics,secType) {document.getElementById("exportCurrent").setAttribute("disabled",true);
        //查询传送的参数
        this.searchParams = {
          ...
          if (rtnStatic == false) {
            ...
            if (dataResponse.length == 0) {
              ...
              if (dataResponse.length == 1) {
                secType == '001' ? _this.tableData1 = [] : _this.tableData2 = [];
              } else {
                secType == '001' ? _this.tableData1 = _this.dataArrayAll :_this.tableData2 = _this.dataArrayAll;
              }
...
<style lang="less">
  #fts00012 .el-table__body-wrapper tr:nth-last-child(1) {
    display: none;
  }
