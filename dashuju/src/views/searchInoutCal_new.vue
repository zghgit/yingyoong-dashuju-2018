<template>
  <section class='constWidth' id="fifteenCommon">
    ...
    <Collapse id="dealPanel" class="dealPanel" v-model="collapse" >
      <Panel name="1" id="searchInoutCal">
        <p>先进先出
          <span id="lookInfoLink" class="lookInfo" @click.stop="lookCountInfo">查看统计口径</span>
        ...
        <div slot="content">
          <!-- 第一个 -->
          <div v-show="currentActiveKey == 0">
            <p slot="title"></p>
            <Form ref="formValidateOne" :model="formValidateOne" :rules="ruleValidateOne">
              <Row class="uploadBox proCalBtnBox">
                <Upload ref="upload" :show-upload-list="false" accept=".xls,.xlsx,.csv" :default-file-list="defaultList"
                        :format="['xls','xlsx','csv']" :headers="upfileParams" :on-success="handleSuccessAccount"
                        :max-size="2048" :on-format-error="handleFormatError" :on-exceeded-size="handleMaxSize"
                        :before-upload="handleBeforeUpload1" :on-error="enableUpload1" multiple
                        action="/dwapp/dwappUpload/decode_upfile?uploadType=TRANDETAIL" style="display: inline-block;">
                ...
                <Modal v-model="lookUploadInfo" title="查看已上传的文件" width="884" class-name="vertical-center-modal"
                       cancel-text>
                  <Table height="300" id="lookInfoTable" border stripe :columns="columnsLookInfo"
                         :data="dataLookInfo"></Table>

                </Modal>
                <Button id="templateBtn" @click="modelType(1)">模板</Button>
                <Modal title="账面盈利计算文件模板" v-model="templateInfo" width="684" class-name="vertical-center-modal"
                       cancel-text>
              ...
              <br>
              <Form-item style="position: relative;left: 20px;top: 15px;">
                <label>佣金费率</label>
                <Input v-model="inputNumberOne" style="width:50%;"></Input>
                <label class="bluetext">&nbsp;&nbsp;%</label>
              </Form-item>
              <Form-item prop="inputOne" id="AccountForm1">
                在页面上显示前&nbsp;&nbsp;
                <Input-number id="inputOne" @on-change="validNum" v-model="formValidateOne.inputOne" :max="2000"
                              :min="1" size="small" style="width: 70px;"></Input-number>&nbsp;&nbsp;行
                <p><label class="redtext fontsize12">{{showrowTip}}</label></p>
                <p>
                  <label class="redtext fontsize12">(最多显示2000行)</label>
                </p>
              </Form-item>
              <Form-item style="margin: 15px 0 0 50px;">
                <div class="cxBox">
                  <Button id="searchBtn" type="primary" @click="handleSubmitOne('formValidateOne')"
                          style="float:right;width:80px;">查询
                  </Button>
                </div>
              </Form-item>
            </Form>
          ...
          <!--第二个-->
          <div v-show="currentActiveKey == 1">
            <p slot="title"></p>
            <Form ref="formValidateTwo" :model="formValidateTwo" :rules="ruleValidateTwo">
              <Row class="uploadBox proCalBtnBox">
                <Upload ref="upload" :show-upload-list="false" accept=".xls,.xlsx,.csv" :default-file-list="defaultList"
                        :format="['xls','csv','xlsx']" :data="uploadParams" :headers="upfileParams" :max-size="2048"
                        :on-format-error="handleFormatError" :on-exceeded-size="handleMaxSize"
                        :before-upload="handleBeforeUpload2" :on-error="enableUpload2"
                        :on-success="handleSuccessAccount2" multiple
                        action="/dwapp/dwappUpload/decode_upfile?uploadType=DETAILCHARGE"
                ...
                <Modal title="查看已上传的文件" v-model="formValidateTwo.modalTwoFirst" width="884"
                       class-name="vertical-center-modal" cancel-text>
                  <Table height="300" id="lookInfoTable" border stripe :columns="columnsLookInfoTwo"
                         :data="dataLookInfoTwo"></Table>


                </Modal>
                <Button @click="modelType(2)" type="ghost">模版</Button>
                <Modal title="账面盈利计算文件模板" v-model="formValidateTwo.modalTwoSecond" width="884"
                       class-name="vertical-center-modal" cancel-text>
              ...
              <Form-item style="margin: 15px 20px 0 20px">
              ...
              <Form-item prop="inputTwo" id="AccountForm2" >
                在页面上显示前&nbsp;&nbsp;
                <Input-number id="inputTwo" @on-change="validNum2" v-model="formValidateTwo.inputTwo" :max="2000"
                              :min="1" size="small" style="width: 70px;"></Input-number>&nbsp;&nbsp;行
              ...
              <Form-item style="margin: 15px 0 0 50px;">
                <Button type="primary" @click="handleSubmitTwo('formValidateTwo')" style="float:right;width:80px;">查询
                </Button>
              </Form-item>
          ...
          <div v-show="currentActiveKey == 2">
            ...
            <Form ref="formValidateThree" :model="formValidateThree" :rules="ruleValidateThree">
              ...
              <Row style="margin-top:10px;" class="uploadBox proCalBtnBox">
                <span><b>文件上传</b><span class="redtext">（先进先出）</span></span>
                <Upload ref="upload" :show-upload-list="false" accept=".xls,.xlsx,.csv" :data="uploadParams1"
                        :headers="upfileParams" :default-file-list="defaultList" :format="['xls','xlsx','csv']"
                        :on-success="handleSuccessAccount3" :max-size="2048" :on-format-error="handleFormatError"
                        :on-exceeded-size="handleMaxSize" :on-error="enableUpload3" :before-upload="handleBeforeUpload3"
                        multiple action="/dwapp/dwappUpload/decode_upfile?uploadType=PROFITFIFO"
                        style="display: inline-block;">
                ...
                <Modal title="查看上传列表" v-model="formValidateThree.modalThreeFirst" width="884"
                       class-name="vertical-center-modal" cancel-text>
                  <Table height="300" id="lookInfoTable" border stripe :columns="columnsLookInfoThreeFirst"
                         :data="dataLookInfoThreeFirst"></Table>


                </Modal>
                <Button @click="modelType(3)" type="ghost">模版</Button>
                <Modal title="模版" v-model="formValidateThree.modalThreeSecond" width="684"
                       class-name="vertical-center-modal" cancel-text>
                  <Table id="templateTable" border stripe :columns="columnsTemplateThreeFirst"
                         :data="dataTemplateThreeFirst"></Table>
              ...
              <Row style="margin-top:10px;" class="uploadBox proCalBtnBox">
                <span><b>文件上传</b><span class="redtext">（后进先出）</span></span>
                <Upload ref="upload" :show-upload-list="false" accept=".xls,.xlsx,.csv" :default-file-list="defaultList"
                        :format="['xls','xlsx','csv']" :max-size="2048" :on-success="handleSuccessAccount4"
                        :data="uploadParams2" :headers="upfileParams" :on-format-error="handleFormatError"
                        :on-exceeded-size="handleMaxSize" :on-error="enableUpload4" :before-upload="handleBeforeUpload4"
                        multiple action="/dwapp/dwappUpload/decode_upfile?uploadType=PROFITLIFO"
                        style="display: inline-block;">
                ...
                <Modal title="查看上传列表" v-model="formValidateThree.modalThreeThird" width="884"
                       class-name="vertical-center-modal" cancel-text>
                  <Table height="300" id="lookInfoTable" border stripe :columns="columnsLookInfoThreeSecond"
                         :data="dataLookInfoThreeSecond"></Table>


                </Modal>
                <Button @click="modelType(4)" type="ghost">模版</Button>
                <Modal title="模版" v-model="formValidateThree.modalThreeForth" width="884"
                       class-name="vertical-center-modal" cancel-text>
                  <Table id="templateTable" border stripe :columns="columnsTemplateThreeSecond"
                         :data="dataTemplateThreeSecond"></Table>
                </Modal>
              </Row>
              ...
              <Form-item prop="inputThree" id="AccountForm3" >
                在页面上显示前&nbsp;&nbsp;
                <Input-number id="inputThree" @on-change="validNum3" v-model="formValidateThree.inputThree" :max="2000"
                              :min="1" size="small" style="width: 70px;"></Input-number>&nbsp;&nbsp;行
              ...
              <Form-item style="margin: 20px 0 0 0px;">
                <Button type="primary" @click="handleSubmitThree('formValidateThree')" style="float:right;width:80px;">
                  查询
                </Button>
              </Form-item>
          ...
          <div v-show="currentActiveKey == 3">
            ...
            <Form ref="formValidateFour" :model="formValidateFour" :rules="ruleValidateFour" style="margin-top:10px">
              <Row class="uploadBox proCalBtnBox">
                <Upload ref="upload" :show-upload-list="false" accept=".xls,.xlsx,.csv" :default-file-list="defaultList"
                        :format="['xls','xlsx','csv']" :max-size="2048" :on-format-error="handleFormatError"
                        :headers="upfileParams" :on-success="handleSuccessAccount5" :on-exceeded-size="handleMaxSize"
                        :before-upload="handleBeforeUpload5" :on-error="enableUpload5" multiple
                        action="/dwapp/dwappUpload/decode_upfile?uploadType=HIGH" style="display: inline-block;">
                ...
                <Modal title="查看上传列表" v-model="formValidateFour.modalFourFirst" width="884"
                       class-name="vertical-center-modal" cancel-text>
                  <Table height="300" id="lookInfoTable" border stripe :columns="columnsLookInfoFour"
                         :data="dataLookInfoFour"></Table>


                </Modal>
                <Button @click="modelType(5)" type="ghost">模版</Button>
                <Modal title="模版" v-model="formValidateFour.modalFourSecond" width="684"
                       class-name="vertical-center-modal" cancel-text>
                  <Table id="templateTable" border stripe :columns="columnsTemplateFour"
                         :data="dataTemplateFour"></Table>
                </Modal>
              </Row>
              <Row style="margin-top:10px; margin-Bottom:10px">
                <p>已上传记录总数:
              ...
              <Form-item prop="inputFour" id="AccountForm4" style="margin:0 20px">
                在页面上显示前&nbsp;&nbsp;
                <Input-number id="inputFourForm" @on-change="validNum4" v-model="formValidateFour.inputFour" :max="2000"
                              :min="1" size="small" style="width: 70px;"></Input-number>&nbsp;&nbsp;行
              ...
              <Form-item>
                <Button type="primary" @click="handleSubmitFour('formValidateFour')" style="float:right;width:80px;">
                  查询
                </Button>
              </Form-item>
            </Form>
    ...
    <Tabs :animated="false" value="" type="card" @on-click="tabsClickHandle" class="inoutCalBox" style="padding:0 30px ">
      <Tab-pane label="输出成交明细和费用" key="0">
        ...
        <Row style="margin-top:-10px; margin-bottom:10px;">
          ...
          <Modal class="modal" title="输出成交明细和费用 统计口径说明" v-model="modalOne" class-name="vertical-center-modal"
                 width='700' cancel-text>
            <h3 class="">
              <b>数据说明</b>
            </h3>
        ...
        <Row style="float:right;">
          <Button id="calExportBtnOne" type="primary" style="width:300px;  margin-top:0px;" @click='exportTable(1)'
                  disabled>导出全部结果为XLSX文件
          </Button>
        </Row>
        <!--</Card>-->

      </Tab-pane>
      <Tab-pane label="输出买卖记录对应数据表" key="1">
        <Row :gutter="gutterValue2">

          <p slot="title">输出买卖记录对应数据表</p>
          <Row style="margin-top:-10px; margin-bottom:10px;">

            <Modal class="modal" title="输出买卖记录对应数据表 统计口径说明" v-model="modalTwo" class-name="vertical-center-modal"
                   cancel-text width='700' height='1000' cancel-text>
              <h3 class="text">
                <b>数据说明</b>
              </h3>
              <p class="textP">无</p>
          ...
          <div style="padding-top:10px">
            <Page :total="returnTotal2" :current="returnN2" :page-size="500" :page-size-opts=[500,1000]
                  placement="top" show-sizer @on-page-size-change="tableChangeSize2" @on-change="tableChange2"></Page>
          </div>

          <Row style="margin-top: 20px;text-align: center;">
            <Button id="calExportBtnTwoFirst" type="primary" style="width:300px;" @click='exportTable(2)' disabled>
              将查询结果导出为XLSX文件（先进先出）
            </Button>
            <Button id="calExportBtnTwoSecond" type="primary" style="width:300px;  margin-left:10px;"
                    @click='exportTable(3)' disabled>将查询结果导出为XLSX文件（后进先出）
            </Button>
          </Row>
      ...
      <Tab-pane label="输出盈利统计表"   key="2">
        ...
        <Row :gutter="gutterValue3">
            ...
            <Row style="margin-top:-10px; margin-bottom:10px;">
              ...
              <Modal class="modal" title="输出盈利统计表 统计口径说明" v-model="modalThree" class-name="vertical-center-modal"
                     cancel-text width='700' height='1000' cancel-text>
                <h3 class="text">
                  <b>数据说明</b>
                </h3>
            ...
            <div style="padding-top:10px">
              <Page :total="returnTotal3" :current="returnN3" :page-size="500" :page-size-opts=[500,1000]
                    placement="top" show-sizer @on-page-size-change="tableChangeSize3" @on-change="tableChange3"></Page>
            </div>
            <Row style="margin-top: 20px;text-align: center;">
              <Button id="calExportBtnThreeFirst" type="primary" @click='exportTable(4)' style="width:300px;" disabled>
                将查询结果导出为XLSX文件（先进先出）
              </Button>
              <Button id="calExportBtnThreeSecond" type="primary" @click='exportTable(5)'
                      style="width:300px; margin-left:10px;" disabled>将查询结果导出为XLSX文件（后进先出）
              </Button>
            </Row>
      ...
      <Tab-pane label="输出高送转明细"  key="3">
        ...
        <Row :gutter="gutterValue4">
            ...
            <Row style="margin-top:-10px; margin-bottom:10px;">
              ...
              <Modal class="modal" title="输出高送转明细 统计口径说明" v-model="modalFour" class-name="vertical-center-modal"
                     cancel-text width='700' height='1000' cancel-text>
                <h3 class="text">
                  <b>数据说明</b>
                </h3>
            ...
            <fastTable :width="930" :bwidth="920" :addWidth="0" :columns="columns4" :data="data4"
                       class="inoutTable2"></fastTable>

            <div style="padding-top:10px">
              <Page :total="returnTotal4" :current="returnN4" :page-size="500" :page-size-opts=[500,1000]
                    placement="top" show-sizer @on-page-size-change="tableChangeSize4" @on-change="tableChange4"></Page>
            </div>


            <Row style="float:right">
              <Button id="calExportBtnFour" type="primary" style="width:300px;  margin-top:0px;" @click='exportTable(6)'
                      disabled>导出全部结果为XLSX文件
              </Button>
            </Row>
...
<script>
  ...
  import {
    ...
    columnsThreeSecond,
    columns4
  } from '../assets/js/searchInoutCal.js';
  import {
    urlParams,
    signatureA,
    srcUsernameA,
  ...
  import '../theme/fourAppCommon.less';
  ...
  export default {
    ...
    methods: {
      //点击查询
      lookCountInfo:function(){
        if(this.currentActiveKey == 0){
          this.modalOne = true
        }else if(this.currentActiveKey == 1){
          this.modalTwo = true
        }else if(this.currentActiveKey == 2){
