<template>
  <section class="constWidth" id="fifteenCommon">
    <div id="gdh00007">
      ...
      <Modal id="lookInfoPop" title="股票质押回购资金使用统计  统计口径说明" v-model="changeInfo"
        ...
        <h3>二、指标说明</h3>
        <p>
          1. 交易额度：为期末上交所批准的会员股票质押式回购额度，同一会员只有一个额度，不区分出资类别
          <br>2. 出资类别：根据融出方账户属性来统计，分别有集合资产、定向资产、专项资产、券商自营、其他资金
        ...
        <p>
          1. 会员名称为全称
          <br>2. 按会员维度、出资类别维度进行汇总统计，根据实际使用PBU代码的会员进行统计，即如果有基金，非会员租用则统计到基金和非会员
          <br>3. 通常情况下，在T日20:00起可访问应用获得包含T日正确统计数据
          <br>4. 某会员、某出资类别、有以下4种数据则展示：
          <br>a) 会员有批准额度
          <br>b) 有期初待购回余额
          <br>c) 有期末待购回余额
          <br>d) 统计期间有交易
          <br>5. 排序方式为：会员名称（正序从小到大）, 出资类别（正序从小到大）
        </p>
      </Modal>
    ...
    <!--<h2 class="infoTitle">股票质押回购资金使用统计</h2>-->
    <!--<div class="mainBox changeBox" id="gdh00007">-->
    ...
    <!--<Modal id="lookInfoPop" title="股票质押回购资金使用统计  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"-->
    <!--cancel-text>-->
    <!--<h3>数据说明</h3>-->
    <!--<p>只包含综合业务平台股票质押式回购统计数据 </p>-->
    <!--<h3>指标说明</h3>-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/GDH_00007.js';
  ...
  export default {
    ...
    methods: {
      ...
      handleSubmit(name, isPageClick) {
        // alert(this.formValidate.enddate)
      ...
      exportData() {
        ...
        void(window.open('/dw/download/gdh00007export?' + downfileParams + commonGetReqParam));
      },
      /**
       * 获取表格汇总行展示信息
       */
      getSumLast() {
        ...
        //   var keyArr = ['mem_name','out_acct_desc','mem_limit','trade_amt','buy_trade_amt','repur_trade_amt','individual_amt',
        //     'especial_amt','org_amt','trade_cnt','stat_trade_cnt','recruit_trade_cnt','repur_trade_cnt','part_trade_cnt','end_cnt'
        //     ,'disobey_cnt','individual_vol','especial_vol','org_vol','stat_amt','end_amt','unmature_amt','end_trade_cnt','individual_cnt'
        //     ,'especial_cnt','org_cnt']
        //
      ...
      formatData (row, column, cellValue) {
        ...
        if (prop !== 'mem_name' && prop !== 'out_acct_desc') {
...
<style lang="less">
  #gdh00007 .el-table__row:nth-last-child(1) {
    display: none;
  }
