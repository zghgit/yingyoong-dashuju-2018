<template>
  <div id="fifteenCommon">
    ...
    <Modal id="lookInfoPop" title="盘后报价回购额度报表  统计口径说明" v-model="changeInfo"
      ...
      <p>
        ...
        <br>3.可用额度：
        T日该会员可用额度=Min[∑(该账户证券明细数据中各对应现券的有效数量×该现券当日折算率×1000)，报价回购额度]-T日收市后未到期报价回购总余额-
        其中，各对应现券的有效数量=max（证券明细中的现券数量-冻结数量，0）
        T日收市后未到期报价回购总余额=T日闭市后正常未到期报价回购总余额+T日新开回购延迟交收部分=∑（未到期数据中辅助代码为ZC且标志为N的记录数量×1000）+∑（未到期数据中辅助代码为YC且统计日期为T日的记录数量×1000）。
      ...
      <!--只包含综合业务平台报价回购数据<br>-->
      <!--<b>二．指标说明</b>-->
      <!--<br>1.批准额度：交易所批准的报价回购总额度。-->
      <!--<br>2.标准券总额度：根据会员质押库的所有无司法瑕疵的质押券根据标准券折算率计算出来的总额度，有司法瑕疵的质押券为登记公司发给本所的证券冻结文件中进行标志的质押券。-->
      <!--<br>3.可用额度：-->
      <!--<br>T日该会员可用额度=Min[∑(该账户证券明细数据中各对应现券的有效数量×该现券当日折算率×1000)，报价回购额度]-T日收市后未到期报价回购总余额。-->
      <!--<br>其中，各对应现券的有效数量=max（证券明细中的现券数量-冻结数量，0）-->
      <!--<br>T日收市后未到期报价回购总余额=T日闭市后正常未到期报价回购总余额+T日新开回购延迟交收部分=∑（未到期数据中辅助代码为ZC且标志为N的记录数量×1000）+∑（未到期数据中辅助代码为YC且统计日期为T日的记录数量×1000）。-->
      <!--<br>4.预警级别：当可用额度占比大于等于20%，预警级别为“安全”；当实时可用额度占比大于等于10%，小于20%的时候，预警级别为“注意”；当实时可用额度占比小于10%；预警级别为“危险”。-->
      <!--<br> <b>三．其他说明</b>-->
      <!--<br>无-->
    </Modal>
...
<script>
  ...
  import {tableColumns1} from '../assets/js/BND_15014.js';
