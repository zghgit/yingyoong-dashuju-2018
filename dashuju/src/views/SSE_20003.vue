<template>
  <section class="constWidth" id="sse20003">
    <div id="fifteenCommon">
      ...
      <div class="tableBox">
        <p v-show="activeName == 'first'" id="infoTableList" class="redtext listlength"></p>
        <p v-show="activeName == 'second'" id="infoTableList2" class="redtext listlength"></p>
        <el-table
          :data="tableData1"
          border
          ...
          <template v-for="(item,index) in tableColumns1">
            <el-table-column v-if="item.hasAnchor" :key="index+(Math.random()*1)"
                             :prop="item.field"
                             :label="item.title"
                             :align="item.align"
                             :width="item.width">
            ...
            <el-table-column v-else :key="index+(Math.random()*1)"
                             :prop="item.field"
                             :label="item.title"
                             :align="item.align"
                             :width="item.width">
    ...
    methods: {
      ...
      anchorClick: function (scope, ind, index) {
        ...
        if (i == 0) {
          ...
          // this.dayTableColumns = (ind ==0 && this.isShowDayAvg == false) ? this.tableColumns1Month : this.tableColumns1Day
        } else {
          if (ind == 0) {
          ...
          // this.dayTableColumns = (ind == 0 && this.isShowDayAvg == false) ? this.tableColumns2Day : this.tableColumns2Month
        }
        ...
        this.currentSelectDate = index
        if (this.activeName == 'second') {
          obj.multiSecCode = scope.row.od
        }
        this.dayTableModel = true
        self.dayTableData = []
        document.querySelector('#exportCurrent2').disabled = true
        $.ajax({
          type: 'POST',
          timeout: 600000,
          headers: commonHeader,
          url: '/dw/mktdt/sse_20003_detail' + (this.activeName == 'first' ? '' : 2),
        ...
        this._fetch(fetch(url, {
          ...
          if (rtnStatic == false) {
            ...
            if (this.tableData1.length == 0 && this.tableData2.length == 0) {
              ...
              console.log(_this.dealitems)
              if(_this.dealitems>0){
                $("#exportCurrent").removeAttr("disabled");
              }
              _this.isHide = true;
              //查询结果提示变化1
              $("#infoTableList").html("查询结果共 " + _this.dealitems + " 条记录");
              $("#infoTableList2").html("查询结果共 " + this.tableData2.length + " 条记录");
              //隐藏加载中
              $("#loadBox").addClass("hide");
            }
          }
      ...
      exportData2() {
        ...
        downfileParams += '&type=' + this.currentSelectDate
        $("#exportCurrent").attr("disabled");
        setTimeout(function () {
          $('#exportCurrent').removeAttr('disabled');
        }, 5000);
