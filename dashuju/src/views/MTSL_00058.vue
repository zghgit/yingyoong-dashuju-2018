<template>
  <section class="constWidth">
    <!--头部logo-->
    <header></header>
    <h2 class="infoTitle">融资融券个股余额情况</h2>
    <div class="mainBox changeBox">
      <Row :gutter="gutterValue">
        ...
        <Card class="iviewCard">
          ...
          <Modal id="lookInfoPop" title="融资融券个股余额情况  统计口径说明" v-model="changeInfo"
                 class-name="vertical-center-modal"
                 cancel-text>
            <h3>数据说明</h3>
            <p>日期区间之间证券每一天的个股余额情况</p>
            <h3>指标说明</h3>
            <p>
              1． 证券简称：T日生效的产品基础信息中该证券的简称。<br>
              2． 融资余额：按照各会员上报的T日沪市“融资融券业务余额汇总统计”数据，统计其中全体沪市证券“融资余额”字段的汇总值（含非当前有效标的数据；且包含非A股品种）。<br>
              3． 融资余额占流通市值比例 = ( 融资余额 / 该证券流通市值 ) * 100%；该证券流通市值 = T日清算后持仓总数量 * 该证券T日收盘价<br>
              4． 融资余额环比增速 = ( ( T日融资余额 / T-1日融资余额 ) – 1 ) * 100%，当且仅当T-1日融资余额为0时，结果为空。<br>
              5． 融券余额：按照各会员上报的T日沪市“融资融券业务余额汇总统计”数据，统计其中全体沪市证券“融券余量金额”字段的汇总值（含非当前有效标的数据；且包含非A股品种）。<br>
              6． 融券余额占流通市值比例 = ( 融券余额 / 该证券流通市值 ) * 100%；该证券流通市值 = T日清算后持仓总数量 * 该证券T日收盘价<br>
              7． 融券余额环比增速 = ( ( T日融券余额 / T-1日融券余额 ) – 1 ) * 100%，当且仅当T-1日融券余额为0时，结果为空。<br>
            </p>
            <h3>其他说明</h3>
            <p>
              1． 假设用户输入的起始日期为T1、结束日期为T2。则本应用展示的日期T日满足：T1 <= T <= T2 。<br>
              2． 正常情况下，预期T日22:30以后可查询T日的数据。因为会员可能于T+1日修改之前T日上报的数据，所以T+1日18:00以后可以查询T日的最终数据。<br>
            </p>
          </Modal>
          <!--头部logo-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/MTSL_00058.js';
