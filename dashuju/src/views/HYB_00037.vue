<template>
  <section class="constWidth" id="fifteenCommon">
    ...
    <div id="hyb00037">
      ...
      <Modal id="lookInfoPop" title="约定购回及股票质押期间回购期限分布  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
             cancel-text>
        <b>一．	整体说明</b>
       <br>1．查询期间有约定购回初始交易或股票质押初始交易的记录才纳入统计。
       <br>2．根据中登系统提供的未到期数据，统计约定购回和股票质押的初始交易金额，仅统计在非担保交收中“交收成功”的数据。
       <br>3．根据“预计购回交易日期-交易日期”算出回购期限。
       <br>4．6个月以内指的是回购期限小于等于182天，6-12月指回购期限大于182天且小于365天；12-36个月指回购期限大于365天。
       <br>5．占比是指3个期限的初始交易金额合计分别占3个期限的初始交易金额合计的比例，3者之和为100%。
      ...
      <!--左右结构代码-->
      <!--<h2 class="infoTitle">约定购回及股票质押期间回购期限分布</h2>-->
      <!--<div class="mainBox changeBox" id="hyb00037">-->
...
<script>
  ...
  import {tableColumns1} from '../assets/js/HYB_00037.js';
  ...
  export default {
    ...
    methods: {
      ...
      fn_formatter:function (row,col,cellValue) {
        ...
        // 需要格式化千分位的字段
        if(prop =='agreed_repurchase_type_securities_trading' || prop == 'stock_pledge_init_trade_amt'){
      ...
      publicPagination(pages, rows, statics) {
        ...
        this._fetch(fetch(url, {
          ...
          if (rtnStatic == false) {
            ...
            var datalength = data.resData.length -1;
      ...
      exportData() {
        ...
        void(window.open('/dw/hyb/hyb_00037_export?' + downfileParams +commonGetReqParam));
      },
      getSumLast() {
        var sums = []
        if(this.tableData1.length<1){
          ...
          var arr = ['stat_type','agreed_repurchase_type_securities_trading','agreed_repurchase_ratio','stock_pledge_init_trade_amt','stock_pledge_ratio']
...
<style lang="less">
  #hyb00037 .el-table__row:nth-last-child(1){
    display: none;
  }
  #hyb00037 ::-webkit-scrollbar {
    width: 7px;
    height: 15px;
  }
  #hyb00037 ::-webkit-scrollbar-thumb {
