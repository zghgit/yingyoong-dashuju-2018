<template>
  <section class="constWidth" @click="ready" id="fifteenCommon">
            ...
            <div>
              <Form-item prop="startdate" id="startDateForm" label="开始日期">
                <Date-picker id="startDate" type="date" placeholder="选择开始日期"
                             v-model="formValidate.startdate"></Date-picker>
              </Form-item>
              <Form-item prop="enddate" id="endDateForm" label="结束日期" style="margin-top: 15px">
            ...
            <Row style="margin-left: 80px;">
              ...
              <Form-item prop="desc" id="AccountForm">
                <Input id="AccountInput" v-model="formValidate.desc" @on-change="accountListChange" type="textarea"
                       :rows="10" placeholder="请输入股东账户"></Input>
              </Form-item>
            </Row>
            <Form-item class="showrow" prop="showrow" id="AccountForm2">
              在页面上显示前&nbsp;&nbsp;<Input-number id="showrowInput" @on-change="validNum" :max="1000" :min="1"
                                               v-model="formValidate.showrow" size="small"
                                               style="width: 70px;"></Input-number>&nbsp;&nbsp;行
    ...
    <div class="containerBox">
      ...
      <Modal id="lookInfoPop" title="庄股协查指标查询  统计口径说明" v-model="changeInfo" class-name="vertical-center-modal"
             cancel-text>
        <h3>数据说明</h3>
        ...
        <h3>其他说明</h3>
        <p>
          在统计时，“会员营业部名称”字段的值基于成交明细数据中的Stat_Seat_Code来确定。如果某一个股东账户在同一交易日内的成交明细数据中出现了不同的Stat_Seat_Code（这种情况比较少见，但存在），那么以该股东账户当天第一笔交易类申报记录对应的Stat_Seat_Code来作为其当天所有交易的Stat_Seat_Code。即对某一个股东账户而言，在同一交易日内所有交易数据的“会员营业部名称”都相同。
        </p>
      </Modal>
      <div class="">
        <Tabs type="card" :animated="false" class="noscrollTabs">
          <Tab-pane label="每日交易、对倒、持仓汇总统计表">
            ...
            <!--    <Table height="516" @on-sort-change="infoTableSort" id="qiTable1" :context="self" :data="TableData3" :columns="qiTableColumns1" size="small" ref="table" border stripe></Table>
            ...
            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal" :current="returnN" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize"
                    @on-change="tableChange"></Page>
            </div>
          </Tab-pane>
          <Tab-pane label="持仓总体统计表">
            <p id="qiList2" class="redtext listlength"></p>
            <!--      <Table height="516" @on-sort-change="infoTableSort" id="qiTable2" :context="self" :data="TableData4" :columns="qiTableColumns2" size="small" ref="table" border stripe></Table>
            <Page :total="returnTotal2" :current="returnN2" :page-size="20" show-elevator show-total @on-change="tableChange2"></Page> -->

            <fastTable :width="950" :bwidth="940" :addWidth="0" :columns="qiTableColumns2"
                       :data="TableData4"></fastTable>

            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal2" :current="returnN2" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize2"
                    @on-change="tableChange2"></Page>
            </div>
          </Tab-pane>
          <Tab-pane label="交易总体统计表">
            <p id="qiList3" class="redtext listlength"></p>
            <!--       <Table height="516" @on-sort-change="infoTableSort" id="qiTable3" :context="self" :data="TableData6" :columns="qiTableColumns3" size="small" ref="table" border stripe></Table>
            <Page :total="returnTotal3" :current="returnN3" :page-size="20" show-elevator show-total @on-change="tableChange3"></Page>  -->
            <fastTable :width="950" :bwidth="933" :addWidth="0" :columns="qiTableColumns3"
                       :data="TableData6"></fastTable>

            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal3" :current="returnN3" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize3"
          ...
          <Tab-pane label="对倒总体统计表">
            ...
            <!--            <Table height="516" @on-sort-change="infoTableSort" id="qiTable4" :context="self" :data="TableData2" :columns="qiTableColumns4" size="small" ref="table" border stripe></Table>
            <Page :total="returnTotal4" :current="returnN4" :page-size="20" show-elevator show-total @on-change="tableChange4"></Page>  -->

            <fastTable :width="950" :bwidth="933" :addWidth="0" :columns="qiTableColumns4"
                       :data="TableData2"></fastTable>

            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal4" :current="returnN4" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize4"
                    @on-change="tableChange4"></Page>
            </div>
          </Tab-pane>
          <Tab-pane label="账户组连续二十个交易日交易占比超过30%统计表">
            <p id="qiList5" class="redtext listlength"></p>
            <!--          <Table height="516" @on-sort-change="infoTableSort" id="qiTable5" :context="self" :data="TableData5" :columns="qiTableColumns5" size="small" ref="table" border stripe></Table>
            <Page :total="returnTotal5" :current="returnN5" :page-size="20" show-elevator show-total @on-change="tableChange5"></Page>  -->

            <fastTable :width="950" :bwidth="933" :addWidth="0" :columns="qiTableColumns5"
                       :data="TableData5"></fastTable>

            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal5" :current="returnN5" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize5"
                    @on-change="tableChange5"></Page>
            </div>

          </Tab-pane>
          <Tab-pane label="账户组连续二十个交易日对倒占比超过20%统计表">
            <p id="qiList6" class="redtext listlength"></p>
            <!--       <Table height="516" @on-sort-change="infoTableSort" id="qiTable6" :context="self" :data="TableData1" :columns="qiTableColumns6" size="small" ref="table" border stripe></Table>
            <Page :total="returnTotal6" :current="returnN6" :page-size="20" show-elevator show-total @on-change="tableChange6"></Page>  -->

            <fastTable :width="950" :bwidth="933" :addWidth="0" :columns="qiTableColumns6"
                       :data="TableData1"></fastTable>

            <div style="padding-top:10px">
              <Page class="pageUtil" :total="returnTotal6" :current="returnN6" :page-size="200"
                    :page-size-opts=[200,500,1000] placement="top" show-sizer @on-page-size-change="tableChangeSize6"
                    @on-change="tableChange6"></Page>
            </div>
